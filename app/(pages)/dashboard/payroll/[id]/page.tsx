...
export default function PayrollExpensesPage() {
  ...
  const paymentScenarios = useMemo(() => {
    ...
    const conservativeScenario = (() => {
      ...
      return {
        ...
        totalCost: cost,
        pros: ['Zero late payment penalties', 'Maintains excellent credit standing', 'Peace of mind with no overdue items'],
        cons: ['Requires immediate cash availability', 'May miss opportunity to optimize cash flow', 'No benefit from payment flexibility'],
    ...
    const balancedScenario = (() => {
      ...
      const mediumTermInsights = expenseInsights.filter(i => {
        ...
        return amount > (salary?.totalBudget || 0) * 0.15;
      });
      ...
      return {
        ...
        totalCost: cost,
        pros: ['Optimizes cash flow management', `Potential savings: ${formatCurrency(savings)}`, 'Balances risk with opportunity'],
        cons: ['Requires careful monitoring of due dates', 'Small risk if payment processing delays occur', 'Need to track multiple payment dates'],
    ...
    const aggressiveScenario = (() => {
      ...
      remainingWithDue.forEach(expense => {
        ...
        paymentPlan.push({
          ...
          paymentOrder: order++,
          suggestedPayDate: dueDate,
          reasoning: `Pay exactly on due date. Maximizes cash availability.`
        });
      });
      ...
      return {
        ...
        totalCost: cost,
        pros: ['Maximizes available working capital', `Theoretical savings: ${formatCurrency(savings)}`, 'Flexibility for unexpected opportunities'],
        cons: ['High risk of late payments if delays occur', 'May damage credit rating if any payment is missed', 'Potential penalties if grace periods misunderstood'],
        recommendationScore: criticalInsights.length > 0 ? 30 : 55
      };
    })();

    // Scenario 4: Smart AI-Driven
    const smartScenario = (() => {
      let cost = 0;
      let savings = 0;
      const paymentPlan: PaymentScenario['expenses'] = [];
      ...
      const analyzeExpense = (expense: Expense): { priority: number; reasoning: string } => {
        ...
        if (insight?.daysUntilDue !== null && insight?.daysUntilDue !== undefined) {
          if (insight.daysUntilDue < 0) { priority += 50; reasons.push(`Overdue by ${Math.abs(insight.daysUntilDue)} days`); }
          else if (insight.daysUntilDue === 0) { priority += 45; reasons.push('Due today'); }
          else if (insight.daysUntilDue <= 3) { priority += 30; reasons.push(`Due in ${insight.daysUntilDue} days`); }
          else if (insight.daysUntilDue <= 7) { priority += 15; reasons.push('Due this week'); }
        } else {
          priority -= 20; reasons.push('No due date - flexible');
        }

        if (percentOfBudget > 25) { priority += 20; reasons.push(`Large expense (${percentOfBudget.toFixed(0)}% of budget)`); }
        else if (percentOfBudget > 15) { priority += 10; reasons.push('Medium-large expense'); }
        else if (percentOfBudget < 2) { priority -= 10; reasons.push('Small expense - low impact'); }
        ...
        if (insight?.daysUntilDue !== null && insight?.daysUntilDue !== undefined && insight.daysUntilDue < 0) {
          ...
          if (penaltyPercent > 5) { priority += 15; reasons.push(`High penalty risk (${formatCurrency(penalty)})`); }
        }

        if (salary && remainingBudget < salary.totalBudget * 0.2) {
          if (amount < 100) { priority += 5; reasons.push('Budget tight - small expense preferred'); }
        }
      ...
      analyzedExpenses.forEach(({ expense, analysis }, index) => {
        ...
        if (analysis.priority >= 90) {
          ...
          const insight = expenseInsights.find(i => i.expense.id === expense.id);
          payDate = insight?.expense.dueDate ? new Date(insight.expense.dueDate) : new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
        ...
        cost += amount;
        paymentPlan.push({ expenseId: expense.id, paymentOrder: order++, suggestedPayDate: payDate, reasoning });
      });

      const optimalTimingSavings = totalExpenses * 0.015;
      ...
      return {
        ...
        riskLevel: 'low' as const,
        totalCost: cost,
        pros: ['AI-powered multi-factor analysis', 'Considers urgency, amount, penalties, and budget', `Optimized savings: ${formatCurrency(savings)}`],
        cons: ['Requires trust in AI recommendations', 'May suggest counterintuitive timing', 'Needs regular monitoring of assumptions'],
  ...
  const onSubmit: SubmitHandler<ExpenseFormData> = async (data) => {
    try {
      ...
      if (editingExpense) {
        ...
        if (response.ok) { toast.success("Expense updated successfully"); fetchExpenses(); setIsDialogOpen(false); }
        ...
        if (response.ok) { toast.success("Expense created successfully"); fetchExpenses(); setIsDialogOpen(false); }
        else { toast.error("Failed to create expense"); }
      }
    } catch (error) {
  ...
  const handleDeleteClick = async (expenseId: number) => {
    ...
    confirm.fire(async () => {
      ...
      if (response.ok) { toast.success("Expense deleted successfully"); fetchExpenses(); }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">Manage and track expenses for your payroll period</p>
        </div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormField control={form.control} name="title" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="e.g., Rent, Utilities, Groceries" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                ...
                {form.watch("amount") > 0 && form.watch("quantity") > 0 && (
                  <div className="p-4 bg-muted rounded-lg border">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-2xl font-bold">{formatCurrency(form.watch("amount") * form.watch("quantity"))}</span>
                    </div>
                  </div>
                )}
                <FormField control={form.control} name="expensesCategoryId" render={({ field }) => (
                  <FormItem>
                ...
                <FormField control={form.control} name="note" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Add any additional details..." className="resize-none min-h-[100px]" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                  <FormField control={form.control} name="withDue" render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between space-y-0">
                      <div className="space-y-1">
                        <FormLabel className="text-base font-semibold">Due Date</FormLabel>
                        <FormDescription>Does this expense have a specific due date?</FormDescription>
                      </div>
        ...
        <div className="space-y-4">
                  ...
                  {criticalInsights.map((insight) => (
                    <div key={insight.expense.id} className="flex items-start justify-between p-2 bg-destructive/10 rounded">
                      <div className="flex-1">
                        ...
                        <p className="text-sm">{insight.message} &bull; {formatCurrency(insight.expense.amount * insight.expense.quantity)}</p>
                        <p className="text-xs mt-1 text-muted-foreground">{insight.action}</p>
                      </div>
                      <Button size="sm" variant="outline" onClick={() => handleOpenDialog(insight.expense)}>Review</Button>
                    </div>
                  ))}
                </div>
              </AlertDescription>
          ...
          {mediumPriorityInsights.length > 0 && criticalInsights.length === 0 && highPriorityInsights.length === 0 && (
            <Alert>
              ...
              <AlertDescription>
                <p className="text-sm">You have {mediumPriorityInsights.length} expense{mediumPriorityInsights.length !== 1 ? 's' : ''} due within the next week.</p>
              </AlertDescription>
            </Alert>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">AI-powered decision tree analysis with {paymentScenarios.length} strategies</p>
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="flex items-center gap-2">
                        {scenario.name}
                        {index === 0 && <Badge variant="default" className="text-xs">Recommended</Badge>}
                      </CardTitle>
                      <CardDescription className="mt-2">{scenario.description}</CardDescription>
                    </div>
                    <Badge variant={scenario.riskLevel === 'low' ? 'default' : scenario.riskLevel === 'medium' ? 'secondary' : 'destructive'} className="ml-2">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-3 bg-green-50 dark:bg-green-950 rounded-lg">
                      ...
                      <p className="text-lg font-bold text-green-600">{formatCurrency(scenario.projectedSavings)}</p>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="w-full bg-muted rounded-full h-2">
                      <div className={`h-2 rounded-full ${scenario.recommendationScore >= 80 ? 'bg-green-500' : scenario.recommendationScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Quick comparison of all payment strategies</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {paymentScenarios.map((scenario) => (
                      <TableRow key={scenario.id}>
                        ...
                        <TableCell className="text-right text-green-600 font-semibold">{formatCurrency(scenario.projectedSavings)}</TableCell>
                        <TableCell className="text-center">
                          <Badge variant={scenario.riskLevel === 'low' ? 'default' : scenario.riskLevel === 'medium' ? 'secondary' : 'destructive'}>
                            {scenario.riskLevel}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-center font-bold">{scenario.recommendationScore}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">{formatCurrency(salary.totalBudget)}</div>
                <p className="text-xs text-muted-foreground mt-1">{salary.month} {salary.year}</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <Badge variant={budgetUtilization > 100 ? "destructive" : budgetUtilization > 80 ? "secondary" : "default"} className="mt-1">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>A detailed breakdown of all expenses for this payroll period</CardDescription>
        </CardHeader>
        <CardContent>
          {expenses.length === 0 ? (
                    ...
                    <TableRow key={expense.id}>
                      ...
                      <TableCell>
                        {expense.expensesCategoryId ? (
                          <Badge variant="outline">{getCategoryName(expense.expensesCategoryId)}</Badge>
                        ) : <span className="text-muted-foreground text-sm">-</span>}
                      </TableCell>
                      <TableCell className="text-right">{formatCurrency(expense.amount)}</TableCell>
                      <TableCell className="text-center"><Badge variant="outline">{expense.quantity}</Badge></TableCell>
                      <TableCell className="text-right font-semibold">{formatCurrency(expense.amount * expense.quantity)}</TableCell>
                      <TableCell className="text-muted-foreground max-w-xs truncate">{expense.note || '-'}</TableCell>
